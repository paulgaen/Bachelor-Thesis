...
int constructNewGraph(Graph& g, vector<EdgeWeight>& in, vector<EdgeWeight>& tot, vector<int>& ntoc){
	...
	vector<std :: unordered_map<int, EdgeWeight> > newGraphEdgeSet (num_comm, std :: unordered_map<int, EdgeWeight>());
	for(Node i = 0; i < g.getNumVertices(); i++){	
	...
	for(vector<std :: unordered_map<int, EdgeWeight> >::iterator it=newGraphEdgeSet.begin(); it!=newGraphEdgeSet.end(); it++){
		for(std :: unordered_map<int, EdgeWeight>::iterator jt=it->begin(); jt!=it->end(); jt++){
